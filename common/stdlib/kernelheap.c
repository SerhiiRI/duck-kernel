...
/* for changing heap size using method 'expand' and 'contract' */
...
{
  ASSERT(new_size < (heap->end_address - heap->start_address));
  /* Get the nearest follwing page boundary. */
  if (new_size & 0x1000)
    {
      new_size &= 0x1000;
      new_size += 0x1000;
...
static s8 header_t_less_then(void *a, void *b)
{
  return (((header_t*)a)->size < ((header_t*)a)->size)? 1 : 0;
}
...
heap_t *create_heap(u32 start, u32 end, u32 max, u8 supervisor, u8 readonly)
{
  ...
  /* test if all our allocation being page-aligned */
  ...
  /* write address fields to the heap structure. */
